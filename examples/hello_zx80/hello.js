...
function SGXinit() {}
function SGXstart(){
	new zx.zx80(gVars.mainSurface, gVars.scale);

	gVars.message_time = 0;
	gVars.message = "...While this is not an attempt to emulate or simulate the ZX80 display, it is a library which can be used to make games in a similar visual style to those retro classics.     ";
...
function SGXdraw() {
	...
	zx.system.screen.print();
	for(var i=0;i<64;++i) {
		zx.system.screen.print(zx.chr$(i+128) + " ", zx.SEMICOLON);
	}
	...
	zx.system.screen.printAt(19,0, zx.chr$(135));
	zx.system.screen.printAt(19,31, zx.chr$(134));
	for(var i=0;i<30;++i) {
		zx.system.screen.printAt(19, i+1, zx.chr$(131));
		zx.system.screen.printAt(21, i+1, zx.chr$(3));
	}
	zx.system.screen.printAt(20,0, zx.chr$(2));
	zx.system.screen.printAt(20,31, zx.chr$(130));
	zx.system.screen.printAt(21,0, zx.chr$(133));
	zx.system.screen.printAt(21,31, zx.chr$(132));
}
